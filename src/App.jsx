...
function VehicleDetail({vehicle:iv,onSave,onBack,showToast,onShowSticker=()=>{},onGetDealer,user='Staff'}) {
  ...
  const myRank=v.listPrice?comps.findIndex(c=>Number(v.listPrice)<=c.price)+1||comps.length+1:null;
      ...
      <Card style={{marginBottom:12,overflow:'hidden'}}>
        ...
        <div style={{padding:16}}>
          ...
          {tab==='pricing'&&<div>
            ...
            {v.marketMid&&<div style={{marginTop:12,background:'#fff',border:`1px solid ${C.border}`,borderRadius:10,overflow:'hidden'}}>
              <div style={{padding:'10px 14px',borderBottom:`1px solid ${C.border}`,display:'flex',alignItems:'center',justifyContent:'space-between'}}><div style={{fontWeight:700,fontSize:13,color:C.navy}}>Competitive Set</div>{myRank&&<span style={{fontSize:11,fontFamily:'monospace',color:C.teal,background:C.tealMuted,padding:'2px 10px',borderRadius:12}}>Rank #{myRank} of {comps.length+1}</span>}</div>
              <table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}><thead><tr style={{background:C.navyMuted}}>{['Rank','Price','KM','Days','Dist.','Dealer'].map(h=><th key={h} style={{padding:'7px 12px',textAlign:'left',fontSize:10,fontWeight:600,color:C.textLight,borderBottom:`1px solid ${C.border}`}}>{h}</th>)}</tr></thead>
              <tbody>{v.listPrice&&<tr style={{background:C.tealMuted}}><td style={{padding:'8px 12px',fontFamily:'monospace',fontWeight:700,color:C.teal}}>#{myRank}</td><td style={{padding:'8px 12px',fontFamily:'monospace',fontWeight:700,color:C.teal}}>{fmt(v.listPrice)}</td><td style={{padding:'8px 12px',fontFamily:'monospace',color:C.teal}}>{fmtN(v.odometer)}</td><td style={{padding:'8px 12px',color:C.teal}}>{days}d</td><td style={{padding:'8px 12px',color:C.teal}}>—</td><td style={{padding:'8px 12px',fontWeight:700,color:C.teal}}>Your Dealership</td></tr>}
              {comps.map((c,i)=><tr key={i} style={{borderTop:`1px solid ${C.border}`}}><td style={{padding:'7px 12px',fontFamily:'monospace',color:C.textMid}}>#{c.rank}</td><td style={{padding:'7px 12px',fontFamily:'monospace',fontWeight:600,color:C.textDark}}>{fmt(c.price)}</td><td style={{padding:'7px 12px',fontFamily:'monospace',color:C.textMid}}>{fmtN(c.odo)}</td><td style={{padding:'7px 12px',color:c.days>45?C.orange:C.textMid}}>{c.days}d</td><td style={{padding:'7px 12px',color:C.textLight}}>{c.dist}km</td><td style={{padding:'7px 12px'}}><div style={{fontWeight:500,fontSize:12,color:C.textDark}}>{c.dealer}</div><div style={{fontSize:10,color:C.textLight}}>{c.city}</div></td></tr>)}
              </tbody></table>
            </div>}
          </div>}
